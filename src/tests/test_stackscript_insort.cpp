...
double test_insort(Interpreter &interpreter, std::stringstream &ss, int size) {
	ss.clear();
...
int main(int argc, char *argv[]) {
	...
	std::ifstream input_file{"./src/programs/insort.txt"};
	...
	for (int i = 0; i < 13; i++) {
		int n = std::pow(2, i);
		double microseconds_per_op = test_insort(interpreter, ss, n);
