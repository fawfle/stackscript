...
double test_queue(Interpreter &interpreter, std::stringstream &ss, int size) {
	ss.clear();
...
int main(int argc, char *argv[]) {
	std::ifstream input_file{"./src/programs/queue.txt"};
	...
	for (int i = 0; i < 15; i++) {
		int n = std::pow(2, i);
		double microseconds_per_op = test_queue(interpreter, ss, n);
